...
/*    OUTPUT: struct3.cpp
	...
	struct3.cpp: In function `void print_part(part*)':
	struct3.cpp:34: request for member `name' in `p', which is of non-aggregate 
	   type `part*'
	struct3.cpp:35: request for member `no' in `p', which is of non-aggregate type 
	   `part*'
	struct3.cpp:36: request for member `price' in `p', which is of non-aggregate 
	   type `part*'
