/****************************************************************************
  ...
  Uses the programmer defined class COMPLEX embedded in this code.
  
  Manually does addition but uses COMPLEX for multiply & subtract
...
void control_cb( int control )
{
  ...
  if ( control == SUBTRACT_ID ) // Uses COMPLEX to do the subtraction
  {
    COMPLEX c1(real1->get_float_val(),imag1->get_float_val());
    COMPLEX c2(real2->get_float_val(),imag2->get_float_val());
    
    COMPLEX cresult = c1 - c2;
    
    resultReal->set_float_val(cresult.getReal());
    resultImag->set_float_val(cresult.getImag());
  }
  
  if ( control == MULTIPLY_ID ) // Uses COMPLEX to do the multiplication
  {
    COMPLEX c1(real1->get_float_val(),imag1->get_float_val());
    COMPLEX c2(real2->get_float_val(),imag2->get_float_val());
...
int main(int argc, char* argv[])
{
  ...
  real1 = glui->add_edittext_to_panel( panel, "A real", GLUI_EDITTEXT_FLOAT);
